...
export function Navbar() {
  ...
  return (
    <nav className="fixed top-0 left-0 w-full z-[100] border-b border-zinc-900 bg-[#050505]/80 backdrop-blur-md">
      <motion.div className="absolute top-0 left-0 right-0 h-[1px] bg-white/40 origin-left" style={{ scaleX }} />
      
      <div className="max-w-7xl mx-auto px-6 h-20 flex justify-between items-center border-x border-zinc-900/50">
        ...
        <div className="flex gap-12 items-center">
          {NAV_LINKS.map((link) => (
            <a 
              ...
              className="nav-link font-sans text-[9px] uppercase tracking-[0.3em] text-zinc-500 hover:text-white transition-colors cursor-none"
            >
              {link.name}
            </a>
          ))}
